r"""Design guarantees and the geographic traps.

The point of the Local Pivotal Method over picking units by hand is that the
result is a PROBABILITY SAMPLE: every unit's inclusion probability is exactly
what was assigned, so 1/pi is a valid design weight and estimates have
defensible standard errors. These tests pin the properties that claim rests on,
plus the two geographic traps that silently break a global design.

CPU only, no network. Standalone:

    python tests/test_geo_and_design.py
...
import lpm_sampling as CS   # noqa: E402
