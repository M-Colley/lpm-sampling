...
__all__ = ["lpm", "lpm1", "lpm2"]

# Probabilities within EPS of 0 or 1 are snapped to 0 or 1. The pair update
# produces residuals like 1e-17 rather than exact zero; without snapping the
# loop would never terminate. The induced bias is bounded by EPS.
EPS = 1e-12
...
def _pivot(pi_i, pi_j, rng):
    ...
    See tests/test_geo_and_design.py::test_inclusion_probabilities_are_respected.
    """
    total = pi_i + pi_j
    if total < 1.0:
